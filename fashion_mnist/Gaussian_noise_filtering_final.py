...
args.epsilon = 0
args.sigma = 0.4
args.model_name = 'Gaussian_noise_NN_final'
...
args.batch_size = train_num
batch_num = int(train_num / args.batch_size)
model_name = 'final_model_sigma_' + str(args.sigma) + '_regparam_' + str(args.reg_param1) + '_epsilon_' + str(args.epsilon)                                                                                                        
...
with open(pathname + '/Final_result.csv', 'a') as newFile:
    headers = ['epsilon', 'sigma', 'reg_param1', 'reg_param2', 'test_loss', 'test_accuracy']
    newFileWriter = csv.DictWriter(newFile, fieldnames=headers)
    newFileWriter.writeheader()
    newFileWriter.writerow({'epsilon': args.epsilon, 'sigma': args.sigma, 'reg_param1': args.reg_param1, 'reg_param2': args.reg_param2, 'test_loss': loss, 'test_accuracy': accuracy})

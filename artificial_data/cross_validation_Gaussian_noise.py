...
## 2. load the data
train_x, val_x, test_x, train_y, val_y, test_y, noisy_S = am_util.load_data(args.data_dir)
...
class NNModel(object):
    ...
    def save(self, sess, model_name, update_num, overwrite=False):
        model_filename = "{}/models/{}.cpkt".format(
            output_dir, am_util.get_name(model_name, update_num)
        )
    ...
    def train_final(self, sess, batch_x, batch_y, update_num, grapher, epsilon, display_step=10):
        batch_x1= am_util.augment_Gaussian_noise(batch_x, args.sigma)
    ...
    for j in range(batch_num):
        ...
        if update_num > args.start_early_stop and update_num % args.display_step == 0:
            ...
            if is_early_stop:
                ...
                with open(args.output_dir + '/cross_validation_Gaussian_noise.csv', 'a') as newFile:
                    headers = ['epsilon', 'sigma, 'val_loss']
                    newFileWriter = csv.DictWriter(newFile, fieldnames=headers)
                    newFileWriter.writeheader()
                    newFileWriter.writerow({'epsilon': args.epsilon, 'sigma': args.sigma, 'val_loss': val_loss})
